...
- **Triage runs when a card enters Triaged** without an active result. If
  ...
  says why: "temporarily unavailable, try again" for an outage, rate limit or
  timeout, or "provider rejected the request" for a bad key or request. A
...
This builds both images and starts the API on [http://localhost:8000](http://localhost:8000) and the
board on [http://localhost:3000](http://localhost:3000). Ctrl-C stops them. Data lives in the
...
provider. The API's interactive docs are at [http://localhost:8000/docs](http://localhost:8000/docs).
...
**CI**: `.github/workflows/ci.yml` runs lint, both test suites, the web build
and typecheck, and `docker compose build` on every push and pull request.
